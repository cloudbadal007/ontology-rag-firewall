...
CONT = Namespace("http://ontoarc.ai/contract#")
...
class ClauseRDFBuilder:
    ...
    def build(self, clause: ExtractedClause, contract_value: float) -> Graph:
        ...
        g = Graph()
        g.bind("cont", CONT)
        contract_uri = URIRef(f"http://ontoarc.ai/instance/contract/{clause.clause_id.split('-')[0]}")
        uri = URIRef(f"http://ontoarc.ai/instance/clause/{clause.clause_id}")
